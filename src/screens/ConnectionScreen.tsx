...
import { EhubLogo } from '../components/EhubLogo';

export function ConnectionScreen({ navigation }: any) {
  const [ipAddress, setIpAddress] = useState('');
  const [token, setToken] = useState('');
  const [focusedField, setFocusedField] = useState<'ip' | 'token' | null>(null);
  ...
  const handleBarcodeScanned = ({ data }: { type: string; data: string }) => {
    ...
    try {
      ...
      if (payload.type === 'ehub-pairing' && payload.urls?.length > 0) {
        ...
        Alert.alert('Invalid QR Code', 'Not a valid Ehub pairing code.');
      }
    } catch {
    ...
    <SafeAreaView style={s.scannerRoot}>
      <View style={s.flex}>
        ...
        <View style={s.scanHintWrap}>
          <Text style={s.scanHintText}>Align the Ehub QR Code</Text>
        </View>
      </View>
    </SafeAreaView>
  );
  ...
  return (
    <SafeAreaView style={s.root}>
      <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.card}>
          ...
          <View style={s.field}>
            <Text style={s.label}>Server Address</Text>
              <View style={[s.row, focusedField === 'ip' && s.rowFocused]}>
                <Server size={18} color={focusedField === 'ip' ? "#3b82f6" : "#64748b"} />
                <TextInput
                  style={s.input}
          ...
          <View style={s.field}>
            <Text style={s.label}>Access Token</Text>
              <View style={[s.row, focusedField === 'token' && s.rowFocused]}>
                <KeyRound size={18} color={focusedField === 'token' ? "#3b82f6" : "#64748b"} />
                <TextInput
                  style={s.input}
        ...
        {/* <Text style={s.footer}>Ehub  ·  MCP-Hybrid SSE+REST</Text> */}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  row: {
    ...
    backgroundColor: '#0f172a', paddingHorizontal: 12, borderRadius: 12,
    height: 48, gap: 10,
    borderWidth: 1, borderColor: '#334155',
  },
  rowFocused: {
