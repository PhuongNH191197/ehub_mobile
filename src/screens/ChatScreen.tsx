...
const MessageItem = React.memo(({ msg, handleDownloadAttachment, baseUrl }: {
  ...
  // Replace ehub://files/ URI with actual HTTP URL for Markdown rendering
  const processedText = React.useMemo(() => {
    if (!msg.text) return '';
    if (!baseUrl) return msg.text;
    return msg.text.replace(/ehub:\/\/files\//g, `${baseUrl}/api/v1/files/download/`);
  }, [msg.text, baseUrl]);
  ...
  const handleDownloadAttachment = React.useCallback(async (attachment: { name: string; base64?: string; url?: string }) => {
    try {
      ...
      if (attachment.url) {
        ...
        if (downloadUrl.startsWith('ehub://files/')) {
          downloadUrl = downloadUrl.replace('ehub://files/', `${url}/api/v1/files/download/`);
        } else if (url) {
    ...
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      >
        ...
        <View style={s.header}>
          ...
          <Text style={s.headerTitle}>Ehub Assistant</Text>
          <View style={{ width: 24 }} />
        </View>
